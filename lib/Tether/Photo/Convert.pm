...
sub START {
  ...
  $heap->{child} = POE::Wheel::Run->new(
    Program => ["convert", "-scale", $self->dimensions, $self->inFile, $self->outFile],    # Program to run.
...
event got_sigchild => sub { 
  ...
  $kernel->post($self->parent, 'ev_converted', $self->outFile, $self->type);
  delete $heap->{child};
